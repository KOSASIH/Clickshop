package com.kosasih.clickshop.web.rest;
...
import com.kosasih.clickshop.IntegrationTest;
import com.kosasih.clickshop.config.EmbeddedKafka;
...
class ClickshopKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/clickshop-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/clickshop-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/clickshop-kafka/unregister"));
                return;
            }
        }
